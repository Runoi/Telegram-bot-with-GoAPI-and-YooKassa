...
async def generate_music_start(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.answer_photo(
                img_startgen,
                caption='''
✅ <b>Активирована нейросеть AVRORA</b> – создание музыки в 2 клика: ритм, голос, исполнение, публикация, продвижение.

🎵 <i>Можешь представить свою песню? Теперь ты можешь её создать легко!</i>

1️⃣ Выбери жанр
2️⃣ Выбери режим
3️⃣ Настрой текст, голос и уникальность
4️⃣ 🎉 ТВОЙ ШЕДЕВР ГОТОВ!
5️⃣ Публикуй и монетизируй творчество

📌 <b>Качество результата зависит от:</b>
- Ваших запросов
- Уровня подписки
...
async def generate_music(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if balance < 1:
            await callback_query.message.answer(
                f'''У вас закончились генерации. 
2️⃣ Баланс: {balance} генераций🧾\n

Пожалуйста пополните подписку, либо используйте бесплатные генерации что мы даем всем хорошим людям.''',
...
✅Оплачивайте через официальные платежные с-мы безопасно. Нам доверяю: Paypal, Sber, Yandex money, СБП, Vk pay и другие.00
...
📍За каждого приглашенного - 0.5 генераций.\n
